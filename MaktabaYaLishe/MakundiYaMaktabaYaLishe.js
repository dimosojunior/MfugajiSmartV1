...
export default function MakundiYaMaktabaYaLishe ({navigation, route}) {
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMaktabaYaLisheContentsView/?id=${id}&page=${current_page}&page_size=2`
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const transportItem = ({item}) => {
    ...
    return (
      ...
      <CustomCard >
            ...
            navigation.navigate('View Makundi Ya Maktaba Ya Lishe', {...item, CategoryName});
...
{ !isPending ? (
 ...
 <View style={[globalStyles.noitemTextContainer,{backgroundColor:COLORS.white}]}>
  <Text style={globalStyles.noitemText}>Hakuna maelezo yoyote yanayohusiana na {CategoryName}!! !
  </Text>
