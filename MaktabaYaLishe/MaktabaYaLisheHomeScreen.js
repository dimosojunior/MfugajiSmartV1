...
export default function MaktabaYaLisheHomeScreen ({navigation, route}) {
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMaktabaYaLisheCategoriesView/?page=${current_page}&page_size=2`
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const transportItem = ({item}) => {
    ...
    return (
      ...
      <CustomCard >
            ...
            navigation.navigate('Makundi Ya Maktaba Ya Lishe', {...item, JinaLaHuduma });
