...
export default function MatumiziYaIndibataHomeScreen ({navigation, route}) {
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMatumiziSahihiYaIndibataView/?page=${current_page}&page_size=2`
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const transportItem = ({item}) => {
    ...
    return (
      ...
      <CustomCard >
            ...
            navigation.navigate('View Matumizi Ya Indibata', {...item, JinaLaHuduma});
...
{ !isPending ? (
 ...
 <View style={[globalStyles.noitemTextContainer,{backgroundColor:COLORS.white}]}>
  <Text style={globalStyles.noitemText}>Hakuna maelezo yoyote yanayohusiana na {JinaLaHuduma}!! !
  </Text>
