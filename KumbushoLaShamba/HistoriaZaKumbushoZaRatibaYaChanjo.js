...
const HistoriaZaKumbushoZaRatibaYaChanjo = ({ navigation }) => {
...
const checkLoggedIn = async () => {
  ...
  const getItems = () => {
   if (endReached) {
    ...
    //setIsPending(true);
    setIsLoading(true);
    const url = `${EndPoint}/GetAllKumbushoLaChanjoView/?page=${current_page}&page_size=2`;
    fetch(url, {
      method: 'GET',
...
{/*{item && item.UmriWaKuku && item.UmriWaKuku.UmriKwaWiki && (
    ...
    {item.AinaYaChanjo && item.AinaYaChanjo.length > 0 && (
      <View style={{
        backgroundColor:'wheat',
        padding:10,
      }}>
  <Text style={globalStyles.VyakulaPriceCartItemsText}>
    Aina za chanjo: <Text style={{ 
      color: 'green', 
      fontFamily: 'Bold',
       }}>
      {item.AinaYaChanjo.map((chanjo) => chanjo.JinaLaChanjo).join(', ')}
    </Text>
  </Text>
  </View>
        )}
...
>Kumbusho zako za ratiba ya chanjo ulizowahi kuweka</Text>
...
export default HistoriaZaKumbushoZaRatibaYaChanjo;
