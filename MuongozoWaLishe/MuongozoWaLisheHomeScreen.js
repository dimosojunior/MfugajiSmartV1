...
export default function MuongozoWaLisheHomeScreen ({navigation, route}) {
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMuongozoWaLisheView/?page=${current_page}&page_size=2`
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const transportItem = ({item}) => {
    ...
    return (
      ...
      <CustomCard >
            ...
            navigation.navigate('View Muongozo Wa Lishe', {...item, JinaLaHuduma});
